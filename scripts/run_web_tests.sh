...
if ! ${JMETER_DIR}/bin/jmeter -n -t testcases/web-bff/tg98_delete.jmx -q ${CONFIG_FILE} -l ${JTL}; then
  echo "警告: tg98_delete.jmx 失败，但继续..."
fi

# tg99_logout_via-form.jmx 会造成系统登出，所以放到最后单独运行
echo "执行 tg99_logout_via-form.jmx (最后执行，会造成系统登出)"
if ! ${JMETER_DIR}/bin/jmeter -n -t testcases/web-bff/tg99_logout_via-form.jmx -q ${CONFIG_FILE} -l ${JTL}; then
